...
- Upload a file with the same name as the cold storage:New files will be stored in the hot storage and displayed on the file storage management; the original file is still stored in cold storage and can be viewed/fetched by clicking on the version record on the right.
:::
